...
def test_fetch_token_features(q1, q2):
    ...
    q1_words = set(
        word for word in q1_tokens
        if word not in STOP_WORDS
    )

    q2_words = set(
        word for word in q2_tokens
        if word not in STOP_WORDS
    )
    ...
    common_stop_count = len(
        q1_stops.intersection(q2_stops)
    )
    ...
    token_features[6] = int(
        q1_tokens[-1] == q2_tokens[-1]
    )

    token_features[7] = int(
        q1_tokens[0] == q2_tokens[0]
    )
...
def query_point_creator(q1, q2, cv):
    ...
    if total_words == 0:
        ...
        input_query.append(
            round(common_words / total_words, 2)
        )

    # Token features
    token_features = test_fetch_token_features(
        q1, q2
    )
    ...
    # Length features
    length_features = test_fetch_length_features(
        q1, q2
    )
